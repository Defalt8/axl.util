...
SerialObjectInfo::SerialObjectInfo(void* _mem_ptr, uint8 _references, uint32 _size, uint32 _object_id) :
	mem_ptr(_mem_ptr),
...
bool SerialObjectInfo::operator==(const SerialObjectInfo& object_info) const
{
...
Serial::Serial() :
	m_object_registry()
{}
const axl::util::ds::UniList<SerialObjectInfo>& Serial::getObjectRegistry() const
{
	return this->m_object_registry;
}
bool Serial::setPointer(uint32 object_id, void* new_ptr)
{
	for(axl::util::ds::UniList<SerialObjectInfo>::Iterator it = m_object_registry.first(); it != m_object_registry.end(); ++it)
	{
...
bool Serial::onSerialize(SerialObjectInfo& object_info)
{
	return true;
}
bool Serial::onDeserialize(SerialObjectInfo object_info)
{
	return true;
}
...
{
	return m_object_registry.insertLast(SerialObjectInfo(mem_ptr, references, size, object_id));
}
bool Serial::Unregister(void* mem_ptr, uint8 references, uint32 size, uint32 object_id)
{
	return m_object_registry.remove(SerialObjectInfo(mem_ptr, references, size, object_id));
